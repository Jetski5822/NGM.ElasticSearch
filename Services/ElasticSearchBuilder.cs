...
namespace NGM.ElasticSearch.Services {
    public class ElasticSearchBuilder : ISearchBuilder {
        ...
        private readonly IList<BaseFilter> _filters = new List<BaseFilter>();

        private QueryDescriptor<object> _query; 

        public ILogger Logger { get; set; }

        public ElasticSearchBuilder(IElasticClient client, string indexName) {
            _client = client;
            ...
            _query = new QueryDescriptor<object>();
        }
        ...
        /*
         * http://nest.azurewebsites.net/concepts/writing-queries.html
         http://stackoverflow.com/questions/15767656/nest-c-sharp-elasticsearch-ecommerce-filter-combination
         * http://betsmartmedia.github.io/gesundheit/query_building_api.html
         * http://stackoverflow.com/questions/13433981/dynamically-creating-a-filter-in-nest
         
         */

        public ISearchBuilder WithField(string field, int value) {
            _filters.Add(Filter<object>.NumericRange(d => d.OnField(field).From(value).To(value).FromExclusive().ToExclusive()));
            return this;
        }

        public ISearchBuilder WithinRange(string field, int? min, int? max, bool includeMin = true, bool includeMax = true) {
            _filters.Add(Filter<object>.NumericRange(d => {
                d.OnField(field).From(min).To(max);
        ...
        private void CreatePendingClause() {
            ...
            if (Math.Abs(_boost - 0) > epsilon) {
                _query.Boosting(b => b.Positive(p => p. _boost));
            }
